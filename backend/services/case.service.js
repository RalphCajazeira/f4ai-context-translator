...
function isLower(s) { return s && s === s.toLowerCase(); }
function isTitleWord(w) { return !!w && w[0] === w[0].toUpperCase() && w.slice(1) === w.slice(1).toLowerCase(); }
function isTitleCase(s) {
  const ws = String(s || '').trim().split(/\s+/);
...
export function projectGlossaryCaseInSentence(original, target, pairs = []) {
  ...
  for (const p of pairs) {
    ...
    // Substitui a ocorrência do alvo na saída pela versão “projected”
